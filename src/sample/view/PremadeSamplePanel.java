...
import sample.model.PremadeQuestion;
...
public class PremadeSamplePanel extends JPanel {
	...
	private PremadeQuestion tempQuestion;
	private ArrayList <PremadeQuestion> refrenceList;
	...
	public PremadeSamplePanel(AppController baseController) {
		...
		currentQuestionID = 0;
		correctAnswers = 0;
		maxQuestions = baseController.getPremadeQuiz().getMaxQuestions();
		refrenceList= new ArrayList<PremadeQuestion>();
		refrenceList = baseController.getPremadeQuiz().getQuestionList();
		tempQuestion = new PremadeQuestion(0,0,0,0,0);
		tempQuestion = refrenceList.get(0);
		...
		btnCheckAwnser.addActionListener(new ActionListener()
				{
					public void actionPerformed(ActionEvent click)
					{
						...
						baseController.premadeButtonPressed();
					}
				});
	}
	...
	{
		System.out.print("whats missing = " + whatsMissing);
		if (whatsMissing == 1)
		{
			factor1.setText("?");
